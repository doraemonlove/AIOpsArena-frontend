  ...
  <div class="min-h-full px-8 py-8">
    <template v-if="selectedNode">
      <div class="mb-6 rounded-2xl border border-light-border bg-light-fill/40 p-5 dark:border-light-border-dark dark:bg-light-fill-dark/40">
        ...
        <div class="mt-4 flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <div class="mt-2 text-3xl font-semibold">{{ selectedNode.name }}</div>
            <div class="mt-2 text-sm text-light-2 dark:text-light-2">
              Inspect this node's infrastructure metrics without the overview list taking over the page.
            </div>
          </div>
          <div class="rounded-2xl bg-white/80 px-4 py-3 text-right dark:bg-black/10">
            <div class="text-xs uppercase tracking-[0.2em] text-light-2 dark:text-light-2">Pods on Node</div>
      ...
      <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
        <section class="rounded-2xl border border-light-border bg-light-fill/50 p-5 dark:border-light-border-dark dark:bg-light-fill-dark/50">
          <div class="mb-4 flex items-center justify-between">
            <div>
              ...
              <div class="text-2xl font-semibold">Workloads Running Here</div>
            </div>
            <div class="text-sm text-light-2 dark:text-light-2">
              {{ selectedNode.instances.length }} pods
            </div>
          </div>
          ...
          >
            <div
              ...
              :key="pod"
              class="rounded-full border border-light-border bg-white/80 px-4 py-2 text-sm dark:border-light-border-dark dark:bg-black/10"
            >
              {{ pod }}
            </div>
            <div
        ...
        <MetricInspector
          title="Node Metrics"
          description="Choose the most useful infrastructure metrics for this node, then compare trends in the chart area below."
          empty-hint="Choose a node card to start"
        />
      </div>

      <ChartsBoard empty-message="Add one or more node metrics to render charts for this selected node." />
    </template>
      ...
      <div class="mb-6 rounded-2xl border border-light-border bg-light-fill/50 p-5 dark:border-light-border-dark dark:bg-light-fill-dark/50">
        <div class="flex items-end justify-between gap-4">
          <div>
            ...
            <div class="text-3xl font-semibold">Node Directory</div>
            <div class="mt-2 text-sm text-light-2 dark:text-light-2">
              Pick a node to open its metrics workspace. The overview stays compact so you can scan the cluster faster.
            </div>
          </div>
          <div class="text-sm text-light-2 dark:text-light-2">
            {{ topologyList.length }} nodes
          </div>
        </div>
      </div>
        ...
        >
          ...
          >
            <div class="flex items-start justify-between gap-3">
              ...
              >
                {{ item.instances.length > 0 ? `${item.instances.length} pods` : 'No pods' }}
              </div>
            </div>
            ...
            <div class="mt-4 space-y-2 text-sm">
              ...
              <template v-else>
                ...
                >
                  +{{ item.instances.length - 3 }} more pods
                </div>
              </template>
            </div>
