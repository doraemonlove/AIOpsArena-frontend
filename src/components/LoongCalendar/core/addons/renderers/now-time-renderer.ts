import type { LoongCalendarCurrentTimeRendererOptions } from '../../../types'
import type { Renderer } from '../loong-addon-renderer'
import { RendererBase } from './renderer-base'

export class NowTimeRenderer extends RendererBase {
  private __options: LoongCalendarCurrentTimeRendererOptions
  constructor(renderer: Renderer, options: LoongCalendarCurrentTimeRendererOptions) {
    super(renderer)
  ...
  render(ctx: CanvasRenderingContext2D): void {
    ...
    // 绘制左侧三角形
    ctx.beginPath()
    ctx.moveTo(lineX, y - this.__options.size) // 第一个顶点
    ctx.lineTo(lineX + this.__options.size * Math.tan(Math.PI / 3), y) // 第二个顶点
    ctx.lineTo(lineX, y + this.__options.size) // 第三个顶点
    ctx.closePath()
    ctx.fill()
    // 绘制右侧三角形
    ctx.beginPath()
    ctx.moveTo(calendarCurrentViewWidth + lineX, y - this.__options.size) // 第一个顶点
    ctx.lineTo(calendarCurrentViewWidth + lineX - this.__options.size * Math.tan(Math.PI / 3), y) // 第二个顶点
    ctx.lineTo(calendarCurrentViewWidth + lineX, y + this.__options.size) // 第三个顶点
    ctx.closePath()
    ctx.fill()
    ctx.restore()
    ...
    const h = this.__options.fontSize + 2
    this.drawCapsuleRect(ctx, 5, y - h / 2, w - 10, h, 5)
    // 绘制时间
