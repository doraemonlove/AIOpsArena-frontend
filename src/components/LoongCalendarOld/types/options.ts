export interface LoongCalendarThemeOptions {
   ...
   * #### colors为数组
   * - 分类为数字：求模取色
   * - 分类为字符串：按照字典序排列，获取位号后，转为分类为数字
   * - 分类数字字符串均存在：按照数字优先，位号加上数字数量后，再求模取色（分类：1,2,7,a,b,c => 1:1, 2:2, 7:7, a:4, b:5, c:6）
   * #### colors为对象
   * - 读取键值取色，不存在则为Object.values(colors)[0]
   */
...
export interface LoongCalendarOptions {
  ...
  themes?: LoongCalendarThemeOptions[]
